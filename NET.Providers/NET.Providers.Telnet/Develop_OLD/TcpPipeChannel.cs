...
namespace NET.Tools.Telnet_DevelopOld
{
	public abstract class TcpPipeChannel : PipeChannel
    {
        ...
        {
            return await this.socket.ReceiveAsync(this.GetArrayByMemory((ReadOnlyMemory<byte>)memory), socketFlags, cancellationToken)
        ...
        protected override async ValueTask<int> SendOverIOAsync(ReadOnlySequence<byte> buffer, CancellationToken cancellationToken)
        {
            if (buffer.IsSingleSegment)
                return await this.socket.SendAsync(this.GetArrayByMemory(buffer.First), SocketFlags.None, cancellationToken)
                                        .ConfigureAwait(false);
            ...
            foreach (var piece in buffer)
            {
                ...
                segments.Add(this.GetArrayByMemory(piece));
            }
