            ...
            while (!cts.IsCancellationRequested)
            {
                ...
                {                    
                    ...
                    if (bytesRead == 0)
                    {
                        if (!this.CloseReason.HasValue)
                            this.CloseReason = Telnet_DevelopOld.CloseReason.RemoteClosing;
                ...
                catch (Exception e)
                {
                    if (!this.IsIgnorableException(e))
                    {
                        ...
                        if (!this.CloseReason.HasValue)
                            this.CloseReason = cts.IsCancellationRequested ? Telnet_DevelopOld.CloseReason.LocalClosing :
                                                                             Telnet_DevelopOld.CloseReason.SocketError; 
                    }
        ...
        protected async Task ReadPipeAsync(PipeReader reader)
        {
            ...
            while (!cts.IsCancellationRequested)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    this.CloseReason = Telnet_DevelopOld.CloseReason.ProtocolError; // close the connection if get a protocol error
                    this.TryClose();
        ...
        private bool ReaderBuffer(ref ReadOnlySequence<byte> buffer, out SequencePosition consumed, out SequencePosition examined)
        {
            ...
            while (true)
            {
                ...
                if (maxPackageLength > 0 && len > maxPackageLength)
                {
                    ...
                    this.CloseReason = Telnet_DevelopOld.CloseReason.ProtocolError;
