...
namespace NET.Tools.Terminal
{
    public class TerminalHelper
    {
        ...
        private static Dictionary<string, EscapeSequence> sequencesByBeginEndChars = null;
        ...
        private static List<EscapeSequence> sequences = new List<EscapeSequence>()
        {
			...
			new EscapeSequence() { Code = "SGR", Sequence = "Esc[Valuem", Description = "Set the format of the screen and text as specified by n" },
			...
			new EscapeSequence() { Code = "DSN", Sequence = "Esc[6n", Description = "Get cursor position" },
			new EscapeSequence() { Code = "CPR", Sequence = "EscLine;ColumnR", Description = "Response: cursor is at v,h" },
			new EscapeSequence() { Code = "DSN", Sequence = "Esc[5n", Description = "Get device status" },
			new EscapeSequence() { Code = "DSN", Sequence = "Esc[0n", Description = "Response: terminal status OK" },
			new EscapeSequence() { Code = "DSN", Sequence = "Esc[3n", Description = "Response: terminal status Failure" },
        ...
        private static Dictionary<string, EscapeSequence> SequencesByBeginEndCharacters
        {
            get
            {
                if (sequencesByBeginEndChars == null)
                {
                    sequencesByBeginEndChars = new Dictionary<string, EscapeSequence>();

                    foreach (EscapeSequence sequence in sequences)
                    {
                        string beginChar = sequence.Sequence.Substring(3, 1);
                        string endChar = sequence.Sequence.Substring(sequence.Sequence.Length - 1, 1);

                        if (!sequencesByBeginEndChars.ContainsKey(beginChar + endChar))
                            sequencesByBeginEndChars.Add(beginChar + endChar, sequence);
                    }
                }

                return sequencesByBeginEndChars;
            }
        }
            ...
            while (index < chars.Length)
            {
                if (chars[index] == (char)27)
                {
                    ...
                    else
                    {
                        ...
                        bool sequenceExists = TerminalHelper.SequencesByBeginEndCharacters.Keys.Contains(beginEnd);
