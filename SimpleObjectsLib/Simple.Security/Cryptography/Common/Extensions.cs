...
using System.Net;
using Simple.Security.Cryptography.Messages;
using Simple.Security.Cryptography.Messages.Connection;
...
namespace Simple.Security.Cryptography.Common
{
    ...
    internal static partial class Extensions
    {
        internal static byte[] ToArray(this GlobalRequestName globalRequestName)
        {
            switch (globalRequestName)
            {
                case GlobalRequestName.TcpIpForward:
                    return CryptoData_OLD.Ascii.GetBytes("tcpip-forward");
                case GlobalRequestName.CancelTcpIpForward:
                    return CryptoData_OLD.Ascii.GetBytes("cancel-tcpip-forward");
                default:
        ...
        internal static byte[] ToArray(this ServiceName serviceName)
        {
            switch (serviceName)
            {
                case ServiceName.UserAuthentication:
                    return CryptoData_OLD.Ascii.GetBytes("ssh-userauth");
                case ServiceName.Connection:
                    return CryptoData_OLD.Ascii.GetBytes("ssh-connection");
                default:
        ...
        {
            var sshServiceName = CryptoData_OLD.Ascii.GetString(data, 0, data.Length);
        ...
        {
            var sshGlobalRequestName = CryptoData_OLD.Ascii.GetString(data, 0, data.Length);
