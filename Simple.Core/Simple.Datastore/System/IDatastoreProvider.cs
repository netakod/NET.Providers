...
namespace Simple.Datastore
{
    public interface IDatastoreProvider : IDatastoreConnection, IDisposable
    {
		...
		List<TKey> GetRecordKeys<TKey>(TableInfo tableInfo, IPropertyModel idPropertyModel); //, string idFieldName);
		...
		IDataReader GetRecord(TableInfo tableInfo, IPropertyModel idPropertyModel, object id, IEnumerable<int>? propertyIndexes = null, Func<int, IPropertyModel>? getPropertyModel = null);
		...
		void DeleteRecord(TableInfo tableInfo, IPropertyModel idPropertyModel, object id);
