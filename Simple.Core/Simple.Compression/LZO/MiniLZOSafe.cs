...
{
    public static class MiniLZOSafe
    {
        ...
        {
            if (src == null)
                throw new ArgumentNullException("src[] was null");

            if (srcCount > src.Length)
                throw new ArgumentOutOfRangeException("src[] has length " + src.Length.ToString() + ", but srcCount was " + srcCount.ToString());
        ...
        public static byte[] Compress(this byte[] src, int srcStart, int srcLength)
        {
            ...
            if (srcStart < 0)
                throw new ArgumentOutOfRangeException("srcStart was " + srcStart.ToString());

            if (srcStart + srcLength > src.Length)
                throw new ArgumentOutOfRangeException("src[] has length " + src.Length.ToString() + ", but srcStart + srcLength was " + (srcStart + srcLength).ToString());
        ...
        private static uint Compress(byte[] src, uint srcstart, uint srcLength, byte[] dst, uint dststart, uint dstlen, uint[] dictNew)
        {
            ...
            else
            {
                ...
                {
                    offset = 0;
                    index = D_INDEX1(src, lip);
                    lpos = lip - (lip - dictNew[index]);

                    if (lpos < srcstart || (offset = (uint)(lip - lpos)) <= 0 || offset > M4_MAX_OFFSET)
                        literal = true;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                length -= M4_MAX_LEN;
                                dst[lop] = (byte)(M4_MARKER | ((offset & 0x4000) >> 11));
                                lop++;
                                while (length > 255)
                                {
                                    length -= 255;
                ...
                dstlen = (uint)(lop - dststart);
                tmp = (uint)(lin_end - lii);
            }
