...
namespace Simple.Modeling
{
	...
	public class ModelElement : IModelElement
	{
		...
		private Hashtable ModelCollectionsByModelType => this.modelCollectionsByModelType ??= new Hashtable();

		private Hashtable ModelDictionariesByModelType => this.modelDictionariesByModelType ??= new Hashtable();

		private Hashtable SecondKeyDictionariesByOriginalDictionary => this.secondKeyModelDictionariesByOriginalDictionary = new Hashtable();
		...
		// TODO: Create this..
		protected ModelDictionary<int, ModelElement> CreateModelDictionary<TEnumType>()
			where TEnumType : class
		{
			var modelDictionary = new ModelDictionary<int, ModelElement>();

			// TODO: 
			if (!typeof(TEnumType).IsEnum)
				throw new ArgumentException("Input type mus be Enum typpe to create model");
			...
			where T : IDictionary<TKey, TFieldType>, new()
		{
			...
			if (typeof(TFieldType) == typeof(ModelElement) || typeof(TFieldType).IsSubclassOf(typeof(ModelElement)))
				getKey = (modelElement) =>
				{
						if (modelElement is ModelElement<TKey> meg)
							return meg.Key;
						else
							return default;
				};
